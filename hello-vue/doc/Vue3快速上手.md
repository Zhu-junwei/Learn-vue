...
- **适用数据类型**：主要用于基本类型的数据（如字符串、数字、布尔值等），但也可以用于对象。

- **作用**：ref 会将一个值封装成一个带有 .value 属性的响应式对象，使得当 .value 的内容发生改变时，会触发视图的更新。

2. `reactive`

- **适用数据类型**：主要用于对象或数组。

- **作用**：reactive 将整个对象转换为深层响应式对象，即对象的每一层级属性都是响应式的，这样可以直接访问和修改对象的属性而不需要 .value。
